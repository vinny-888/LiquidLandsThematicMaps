...
function getCities(item){
    ...
    citiesWithItem.forEach((city)=>{
        ...
        let classVal = '';
        if(highlight.indexOf(city.id) != -1){
            classVal = '';
        }
        citiesHTML += `<div>${building.count} x <a class="${classVal}" href="https://liquidlands.io/city/${city.id}/_xy${building.x},${building.y}" target="_blank">${city.name}</a></div>`;
    })
    return citiesHTML;
}
